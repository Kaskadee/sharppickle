...
namespace sharppickle.Internal {
    ...
    internal static partial class PickleOperations {
        ...
        public static void PushShortBinaryUnicode(Stack stack, BinaryReader reader) {
        ...
        public static void PushBinaryUnicode8(Stack stack, BinaryReader reader) {
            checked {
                var length = reader.ReadLittleEndianNumber(8);
                stack.Push(Encoding.UTF8.GetString(reader.ReadBytes((int)length)));
            }
        }
        ...
        public static void PushBinaryBytes8(Stack stack, BinaryReader reader) {
            checked {
                var length = reader.ReadLittleEndianNumber(8);
                stack.Push(reader.ReadBytes((int)length));
            }
        }
        ...
        public static void AddItems(Stack stack) {
            var elements = PopMark(stack);
            var set = stack.Peek<ICollection<object?>>();
        ...
        /// <summary>
        ///     Pushes the type of a python object (which is mapped to a registered proxy object) with module and type name read from the stack as a string.
        /// </summary>
        /// <param name="stack">The <see cref="Stack"/> to perform the operation on.</param>
